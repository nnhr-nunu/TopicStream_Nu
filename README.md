...
5. 既定モデルは `gemini-3.5-flash`。`gemini-2.0-flash` は廃止済みなので、404 なら `gemini-2.5-flash` → `gemini-flash-latest` → 3.x の順で切り替える
6. 短いURL: Project → **Domains** で `好きな名前.vercel.app` か自分のドメインを追加する。これが共有用です
